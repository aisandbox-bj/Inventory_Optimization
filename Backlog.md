...
- **APP-OPI-01 — Open-procurement indicator** — a mini **3-lamp traffic light** on the detail-panel header, just **left of the classifier pill** (grey/blue/orange/red), with one lamp each for **PR · PO · In Transit**. Each lamp lights when that state is open (dim/grey when none); hover = summary, click = popover listing each item (number · created date · qty · status · age in days).
  - **Definitions:** **Open PR** = active PR, no PO, not cancelled (`PR_ONLY` chain). **Open PO** = PO raised, not yet at the 3PL (on order from vendor). **In Transit** = received at the 3PL (MB51 mvt **107**) but not yet at site (no **109**). The In-Transit lamp closes the *"low SOH + no open PO but a pile in transit"* false-alarm gap — SAP may close the PO at the 3PL GR, so the PO lamp goes dark while In-Transit lights up.
  - **Data source (both):** per-item detail + states from `TracePhase.computeChains` (PR_ONLY · PO-without-107 · 107-without-109); current authoritative **quantities** from **Inventory Master** `inTransit` + `openPO` (point-in-time SAP snapshot, already parsed). The two should agree — divergence is a data-quality signal (surface it, don't hide). Lamp hidden/grey when its source isn't loaded (PR History for chains; Inventory Master for qtys). Note: Inventory Master `openPO` is plant-conditional (GM=1130 / MLA=1120 3PL hub) — resolve carefully at build.
  - Placement: Trend + Screener detail-header; Trace banner a candidate (no classifier there). Reuses the Screener's existing PO/PR-open logic (APP-SCR-01d). Confirm-at-build: lamp colours; whether to show qty on/by each lamp; Trace placement. No SCHEMA_VERSION bump.
- **APP-PD-SPREAD — chevron total-duration spread** — the A–D chevron's **Total to site** currently shows only the **mean**. Add the spread from the box-plot box of the per-chain **total-to-site** durations (phases A–D, i.e. excluding phase E / first consumption): show the mean with **+ (Q3 − mean)** as a superscript and **− (mean − Q1)** as a subscript (e.g. `35.2d` with a small ⁺⁵ / ₋₄), or a single "± Xd" if the operator prefers. Cheap: chains now carry `totalToSite` (APP-FIX-SIGMA-PROC), so it's `boxStats(chains.map(c => c.totalToSite))` — same `boxStats` already used for the box plots. Applies to the Phase Distribution chevron; the YoY per-year "Total to site" chevrons are a candidate too. Pure display, no data/schema change. Confirm-at-build: asymmetric +/− (super/sub) vs symmetric ± ; which stat for ± (half-IQR vs SD).
