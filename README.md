...
**v2.2.0-dev** — current dev tip (v2.1.5-dev now frozen as the rollback snapshot); last released tag `v2.1.1` (roll back with a clone + `git checkout v2.1.1`). Canonical `SCHEMA_VERSION` = `1.0.0`, `APP_VERSION` = `2.2.0-dev`. **Latest (2026-09-25, origin/main `7d53223`):** the report now has a **docked comment editor in the preview** (write while you look; ↻ Update re-renders), comments are **kept per SAP material in a durable browser store** (`shared/comment-store.js`) so they survive deleting the JSON and carry across assessments, with a **💬 Comments export/import backup** on the Screener toolbar; the preview is a **movable / resizable / maximizable workbench**; and the **MRP-run cadence** (both graphs, `shared/mrp-cadence.js`) is a report block, weekly, on the Trend chart's date range. Rollback tags `checkpoint/pre-livecomment` @ `dbb654d`, `checkpoint/pre-pvworkbench` @ `be14928`. **In progress:** #3 Duplicate-flag (Intake upload → Trend "Classification" tab → All Duplicates / Duplicate Families). **v2.2.0-dev (2026-09-25) first shipped the Screener Report Builder** (customisable per-material report — page size, drag-reorder block picker, comment; US-Letter multi-page with section separators, tables-never-split and cancelled-PR red strikethrough; Widescreen 16:9 drag-and-drop layout canvas with ✕-delete / +add tiles, batch print, and a **Dark = exact on-screen grab** of the real MaterialDetail / TracePhase components), plus **APP-TREND-FILTERHOLD** (Trend filters survive the Trace round-trip) and **APP-FIX-TRACE-NOHIST** (Trace-it on a no-PR-history part says so instead of jumping to another material). Rollback: `backup/pre-v2.2.0` + `checkpoint/pre-v2.2.0` at `00ccee7`. **Prior origin/main = `54cdee1`** (2026-08-17 code `cf5f31f` + doc follow-up; the 2026-08-18 template/Phase-0 pass lands on top — confirm against the newest `record-of-change.html` entry, don't hard-pin). *(Earlier: the whole 2026-08-15 dev day pushed as three area-grouped commits — Trace `0f342be` · Trend+shared `8d3f2ae` · Docs `74baea5` — plus a doc-accuracy follow-up `1d83614`; no SCHEMA_VERSION change). **The "16-Aug feedback pass" (2026-08-16) is pushed as `76b521f`** on top of `1d83614` (toggle fix · cadence per-material · lead-time in DAYS + colour + the Trend/Screener/Sandbox calc tie-up · banner 3-col · fleet cross-fleet · MRP-template tweaks · dashboard Upload-JSON · notes-drag · pop-out aspect/notes-grow · sandbox analyst), followed by the **"Total cost to Max" column** (`e3bbf5e`) and the **"16-Aug rev02 round-2" pass** (pushed on top of `88112ae`): MRP-run **cadence rebuilt** (chain-state 3-outcome · continuous axis · never-exclude), analyst work moved to the **Screener** report, pop-out **fixed-content resize**, hover-helper **ⓘ** cleanup, notes **free-drag**, Excel **notes block**; the sigma trim was root-caused + parked.)* **Since then, pushed on top:** `ccfd895` **"round-3"** (MRP-cadence allocation reconciled + month axis + MRP/Manual hover table · analyst data-loss fix · Calibre nav rebrand · hover sweep) and `2f5e981` **"post-round-3"** (Trace Raw Data **MRP/Manual Trigger** column · **robust median+MAD sigma trim** — the parked sigma item now **fixed** · global native-tooltip suppression `no-hovers.js`), then `838a93f` **"Screener alignment + graph enhancements"** (Screener→Trend read-only detail alignment · dotted-red Min/SS line on the Trend chart · right-edge-resizable MRP-cadence tile), then `275a54d` **"MRP-cadence dual-chart overhaul"** (whole block in one resizable tile · per-day stock-status **dot strip** · new **replenishment chart** stock+PO vs Min/Max · day/month/year axis · always-ends-today ≥3-month span), then `cf5f31f` **"cadence graph pass 3"** (bars → **PR → PO / Cancelled** · stacked **day / month / year** axis showing every month · **blue stock / green PO / hatched-green manual / red cancelled dots** · all helpers **click-triggered** + native-styled · empty-slot "Stock: 0" fixed). **Parked:** FRED-style date-range slider. **Next:** an Excel per-material export that drops in the Screener report visual + a copyable data table (with a max-size multi-file split + on-export warning). All work is **pending operator off-repo validation** (test procedures in the handover folder). Full history + rollback steps live in [`record-of-change.html`](record-of-change.html); operator manual in [`user-manual.html`](user-manual.html).

Highlights since v1.x:
- **Screener Report Builder** (APP-SCR-REPORT, v2.2.0-dev) — build a customisable report for the whole flagged set: US Letter (multi-page, tables never split, cancelled PRs in red strikethrough) or Widescreen 16:9 (drag-and-drop layout canvas, batch Print set, Light report style or Dark exact on-screen grab). Blocks include the Trend graph, Trace supply duration / YoY / last-X PRs / chains, the **MRP-run cadence** (both graphs) and a **Comment** that is written in the preview and kept per material across assessments (with an export/import backup). Renders in-app in a movable, resizable preview.
...
```
canonical-schema.js   Schema, FACTORY_DEFAULTS, PARAMETER_DESCRIPTIONS (SCHEMA_VERSION 1.0.0, APP_VERSION 2.2.0-dev)
...
```bash
cd "4 - Build Output/Inventory Optimization App/v2.2.0-dev"
...
Pushes to GitHub `aisandbox-bj/Inventory_Optimization` (this repo root mirrors `v2.2.0-dev/`, excluding `_rollback/`). Per-change rollback snapshots live in `_rollback/`; the last released tag is `v2.1.1`, and the pre-push tip of each chunk gets a remote `backup/pre-*` branch + `checkpoint/pre-*` tag. Push protocol (clone-to-tmp, identity flags) is documented in `HANDOVER.md`.
